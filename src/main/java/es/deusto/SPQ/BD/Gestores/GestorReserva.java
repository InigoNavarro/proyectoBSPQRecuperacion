...
public class GestorReserva extends GestorBD {
	...
	public static List<Reserva> selectReservasSocio(String socioNombreApellido) {
		PersistenceManager pm = GestorBD.getPMF().getPersistenceManager();
		Transaction transaction = null;
		transaction = pm.currentTransaction();
		try {
			...
			List<Reserva> res = q.executeList();
			transaction.commit();
			if (res.size() > 1) {
	...
	public static List<Reserva> selectReservasJuego(String juegoNombre) {
		PersistenceManager pm = GestorBD.getPMF().getPersistenceManager();
		Transaction transaction = null;
		transaction = pm.currentTransaction();
		try {
